...
async def statistic_in_russia(message: types.Message):
    await bot.send_message(message.from_user.id, get_data("russia", "России 🇷🇺"))


@dp.message_handler(text="Статистика в мире 🌍", state=user_states.countries_page)
async def statistic_in_world(message: types.Message):
    await bot.send_message(message.from_user.id, get_data("world", ""))
